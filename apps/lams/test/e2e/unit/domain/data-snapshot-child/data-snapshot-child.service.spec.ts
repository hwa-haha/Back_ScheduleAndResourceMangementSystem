/**
 * 52100 단위 테스트: DomainDataSnapshotChildService (공개 메서드별 검증)
 */
import { Test, TestingModule } from '@nestjs/testing';
import { getRepositoryToken } from '@nestjs/typeorm';
import { NotFoundException } from '@nestjs/common';
import { DomainDataSnapshotChildService } from '../../../../../src/domain/data-snapshot-child/data-snapshot-child.service';
...
describe('DomainDataSnapshotChildService', () => {
    let service: DomainDataSnapshotChildService;
    ...
    beforeEach(async () => {
        ...
        const module: TestingModule = await Test.createTestingModule({
            providers: [
                ...
                { provide: getRepositoryToken(DataSnapshotChild), useValue: mockRepository },
            ],
        }).compile();
        service = module.get<DomainDataSnapshotChildService>(DomainDataSnapshotChildService);
    });

    describe('생성한다', () => {
        it('저장 후 DTO 반환', async () => {
            const saved = { id: 'c-1', DTO변환한다: () => ({ id: 'c-1' }) };
    ...
    describe('ID로조회한다', () => {
        it('존재하지 않는 id 시 NotFoundException', async () => {
            ...
            await expect(service.ID로조회한다('00000000-0000-0000-0000-000000000000')).rejects.toThrow(/데이터 스냅샷 자식을 찾을 수 없습니다/);
        });
    ...
    describe('수정한다', () => {
        ...
        it('유효한 id 시 저장 후 DTO 반환', async () => {
            ...
            const result = await service.수정한다('c-1', { employeeName: '수정명' }, 'user-1');
            expect(result).toBeDefined();
    ...
    describe('완전삭제한다', () => {
        ...
        it('유효한 id 시 remove 호출', async () => {
            const existing = { id: 'c-1' };
            mockRepository.findOne.mockResolvedValue(existing);
            mockRepository.remove.mockResolvedValue(undefined);
            await service.완전삭제한다('c-1', 'user-1');
