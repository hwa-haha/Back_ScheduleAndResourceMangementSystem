/**
 * 52100 단위 테스트: DomainAttendanceTypeService (공개 메서드별 검증)
 */
...
describe('DomainAttendanceTypeService', () => {
    let service: DomainAttendanceTypeService;
    ...
    beforeEach(async () => {
        ...
        const module: TestingModule = await Test.createTestingModule({
            providers: [
                ...
                { provide: getRepositoryToken(AttendanceType), useValue: mockRepository },
            ],
        }).compile();
        service = module.get<DomainAttendanceTypeService>(DomainAttendanceTypeService);
    });

    describe('생성한다', () => {
        it('저장 후 DTO 반환', async () => {
            const saved = { id: 'at-1', DTO변환한다: () => ({ id: 'at-1' }) };
            mockRepository.save.mockResolvedValue(saved);
            const result = await service.생성한다({ title: '지각', code: 'LATE', workTime: 0, isRecognizedWorkTime: false });
    ...
    describe('ID로조회한다', () => {
        it('존재하지 않는 id 시 NotFoundException', async () => {
            ...
            await expect(service.ID로조회한다('00000000-0000-0000-0000-000000000000')).rejects.toThrow(/출석 타입을 찾을 수 없습니다/);
        });
        it('유효한 id 시 DTO 반환', async () => {
            const entity = { id: 'uuid-1', name: '지각', DTO변환한다: () => ({ id: 'uuid-1', name: '지각' }) };
    ...
    describe('수정한다', () => {
        it('존재하지 않는 id 시 NotFoundException', async () => {
            mockRepository.findOne.mockResolvedValue(null);
            await expect(
                service.수정한다('00000000-0000-0000-0000-000000000000', { title: '수정' }, 'user-1'),
        ...
        it('유효한 id 시 저장 후 DTO 반환', async () => {
            ...
            const result = await service.수정한다('at-1', { title: '수정제목' }, 'user-1');
            expect(result).toBeDefined();
    ...
    describe('삭제한다', () => {
        ...
        it('유효한 id 시 save 호출', async () => {
            ...
            await service.삭제한다('at-1', 'user-1');
    ...
    describe('완전삭제한다', () => {
        ...
        it('유효한 id 시 remove 호출', async () => {
            const existing = { id: 'at-1' };
            mockRepository.findOne.mockResolvedValue(existing);
            mockRepository.remove.mockResolvedValue(undefined);
            await service.완전삭제한다('at-1', 'user-1');
