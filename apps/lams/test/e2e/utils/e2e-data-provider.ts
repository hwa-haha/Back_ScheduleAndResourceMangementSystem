...
/**
 * e2e 테스트에서 API 호출에 필요한 엔티티 ID를 DB 조회로 준비 (52200·52300)
 */
export async function e2e데이터ID를준비한다(
    ...
    if (employeeIds.length > 0) {
        const monthly = await repo.findOne(MonthlyEventSummary, {
            where: { employee_id: employeeIds[0], yyyymm },
        });
        if (monthly) {
    ...
    const file = await repo.findOne(File, { where: { year, month, deleted_at: IsNull() } });
    ...
    const issue = await repo.findOne(AttendanceIssue, { where: { deleted_at: IsNull() } });
    const attendanceIssueId = issue?.id ?? null;

    const project = await repo.findOne(Project, { where: { deleted_at: IsNull() } });
    ...
    const holiday = await repo.findOne(HolidayInfo, { where: { deleted_at: IsNull() } });
    const holidayId = holiday?.id ?? null;

    const wto = await repo.findOne(WorkTimeOverride, { where: { deleted_at: IsNull() } });
    const workTimeOverrideId = wto?.id ?? null;

    const attType = await repo.findOne(AttendanceType, { where: { deleted_at: IsNull() } });
