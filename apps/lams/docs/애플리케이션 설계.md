...
| 레이어           | 설명                                                                                                                                              | 위치 (apps/lams/src 기준)                                                                                                                                                                                                                                                  |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Interface**    | HTTP API 노출, DTO·유효검사, Swagger 문서. 컨트롤러만 두며 비즈니스 서비스 호출.                                                                  | `interface/attendance-data`, `interface/settings`, `interface/attendance-issue`, `interface/approval`, `interface/user`, `interface/dashboard`, `interface/file-management`, `interface/organization-management`, `interface/work-hours`                                   |
| **Business**     | 유스케이스 오케스트레이션. 여러 Context/다른 Business 조합, 트랜잭션 경계·사전 검증.                                                              | `business/attendance-data-business`, `business/settings-business`, `business/approval-business`, `business/user-business`, `business/dashboard-business`, `business/file-management-business`, `business/organization-management-business`, `business/work-hours-business` |
| **Context**      | CQRS Command/Query Handler. 도메인 서비스·리포지토리 조합, 단위 비즈니스 흐름.                                                                    | `context/attendance-data-context`, `context/settings-context`, `context/approval-context`, `context/data-snapshot-context`, `context/dashboard-context`, `context/file-management-context`, `context/organization-management-context`, `context/work-hours-context`        |
| **Domain**       | 엔티티, 도메인 서비스, 비즈니스 규칙(불변성 검증 등). DB 의존만 가짐. LAMS 전용 도메인은 `domain/` 하위, 직원·부서 등 공통은 `libs/modules` 참조. | `domain/daily-event-summary`, `domain/holiday-info`, `domain/attendance-issue`, `domain/employee-department-permission`, `domain/data-snapshot-info` 등                                                                                                                    |
| **Integrations** | 외부 연동. 공휴일 API 동기화, Storage(S3/Local), 조직 마이그레이션(PrvDbMgr), 초기화(Init), Excel 리더, Migration.                                | `integrations/holiday-sync`, `integrations/s3-storage`, `integrations/local-storage`, `integrations/init`, `integrations/migration`, `integrations/prv-db-mgr`, `integrations/excel-reader`, `integrations/storage`                                                        |

**데이터 흐름 요약**: 클라이언트 → Controller → BusinessService → ContextService(CommandBus/QueryBus) → Handler → DomainService / Repository → DB.

**디렉터리 구조 요약**

- **apps/lams/src**: `interface/`, `business/`, `context/`, `domain/`, `integrations/`, `app.module.ts`
- **libs/** (모노레포 루트): `configs/`, `database/`, `integrations/`, `modules/` — LAMS는 `@libs/modules`의 직원·부서·배치이력 등 공통 모듈을 사용.
- **libs/modules**: `department`, `department-history`, `employee`, `employee-department-position`, `employee-department-position-history`, `position`, `rank`

### 4.2 모듈 구성

| 모듈명                     | 설명                                                                                                                                                                                         |
| -------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **AttendanceData**         | 월간/일간 요약 조회·수정, 스냅샷 저장·복원·목록·상세, 월간 요약 노트. 내부: 일간 요약 생성·소프트 삭제·근태 이슈 생성, 일간 재판정(휴일·주말 반영)·월간 요약 생성. DataSnapshotContext 활용. |
| **AttendanceIssue**        | 근태 이슈 목록·부서별·상세, 설명 수정, 반영(확인자=userId), 미반영, 재요청.                                                                                                                  |
| **Settings**               | 휴일·특별근태시간·근태유형 CRUD. 권한(부서·직원·권한 변경·퇴사 여부 플래그), 직원 추가정보. 휴일/특별근태 변경·삭제 시 재판정·월간 요약 생성 호출.                                           |
| **WorkHours**              | 근무시간 월별 조회, 프로젝트 배정·삭제, 근무시간 입력·삭제, 급여계산유형.                                                                                                                    |
| **FileManagement**         | 파일 업로드·반영·이력·복원·목록·다운로드·삭제.                                                                                                                                               |
| **OrganizationManagement** | 부서 목록, 직원 ID 조회 등.                                                                                                                                                                  |
| **Dashboard**              | 부서/직원 기준 집계·대시보드 API(월평균 근무시간, 월별 직원 근태, 주간 상위 직원, 스냅샷, 직원 근태 상세).                                                                                   |
| **Approval**               | 결재 관련 부서별 권한자(검토 권한자) 조회. 퇴사자 제외 목록 등.                                                                                                                              |
| **User**                   | 사용자·연월별 제출 스냅샷 목록, 확정 월간 리포트, 검토 대상 근태 이슈 등. DataSnapshotContext(제출연월 기준 조회) 활용.                                                                      |
| **DataSnapshot**           | 스냅샷 저장·목록(연월/제출연월)·상세·직원별 존재 여부. AttendanceData·User에서 사용.                                                                                                         |
...
| EmployeeDepartmentPermission         | 직원-부서 권한(보기/검토).                                                                                                                                                                  |
| EmployeeExtraInfo                    | 직원 추가정보(대시보드 요약 제외 여부 등).                                                                                                                                                  |
| Employee, Department (libs/modules)  | 직원·부서(공통 모듈). `libs/modules`: employee, department, department-history, employee-department-position, employee-department-position-history, position, rank.                         |
...
| GET                   | /settings/permissions/departments/:departmentId   | 부서별 직원 권한 목록                       |
...
| **Approval**          |                                                   |                                             |
| GET                   | /approval/reviewers-by-department                 | 부서별 검토 권한자(권한자) 조회             |
| PATCH                 | /approval/snapshots/:snapshotId/approval          | 스냅샷 승인(제출) 처리                      |
| GET                   | /approval/snapshots/:snapshotId/content           | 스냅샷 내용 조회                            |
| **User**              |                                                   |                                             |
| GET                   | /user/attendance-issues-to-review                 | 검토 대상 근태 이슈 조회                    |
| GET                   | /user/confirmed-monthly-report                    | 확정 월간 리포트 조회                       |
| GET                   | /user/monthly-report-existence                    | 월간 리포트 존재 여부                       |
| GET                   | /user/monthly-submitted-snapshots                 | 연월별 제출된 최신 스냅샷 목록              |
...
- **실행**: `npm run start:lams` (로컬 개발 감시), `nest start lams` (프로덕션 빌드 후 실행).
- **환경 구분**: 개발 서버와 실 서버를 분리하여 운영. 동일한 환경(EC2 + PM2)에서 각각 PM2로 실행.
- **배포 절차**: 개발 서버에 먼저 배포한 뒤 서버 상태·동작을 확인하고, 이상 없을 때 실 서버에 배포. 빌드 산출물 `dist/apps/lams/main.js`를 PM2 앱(`back-attendance-management-system`)으로 실행. 설정은 루트 `ecosystem.config.js` 참고(instances, env_file, error_file, out_file, PORT 등).
- **로그**: PM2 로그 파일(error_file, out_file) 및 NestJS Logger. log_date_format, merge_logs 등은 ecosystem.config.js에 정의.
- **장애 대응**: PM2 autorestart·max_memory_restart, 로그·헬스체크·알림 연동 등 운영 정책에 따름.
...
| (최종 수정) | 0.3  | 현재 프로젝트 구조 반영. 4.1 레이어별 경로(Interface/Business/Context/Domain/Integrations) 및 디렉터리 구조(apps/lams/src, libs, libs/modules) 정리. 4.2 Approval·User·DataSnapshot 모듈 추가. 설정 권한 퇴사 여부·부서별 직원 권한 API 보강. 6.1 libs/modules 엔티티 명시. 7.1 Approval·User API 목록 추가. |
